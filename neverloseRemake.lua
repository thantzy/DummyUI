...
local CoreGui = (gethui and gethui()) or (get_hidden_gui and get_hidden_gui()) or cloneref(game:FindFirstChild('CoreGui')) or (LocalPlayer and cloneref(LocalPlayer:WaitForChild('PlayerGui', 5)));
...
GlobalWindow.Parent = CoreGui or (LocalPlayer and LocalPlayer:WaitForChild('PlayerGui', 5));
...
function NeverLose:RegisiterHandler(Handler , Signal)
	...
	local function resolveFlag(Config, ctrlType)
		if Config.Flag and Config.Flag ~= "" then
			return Config.Flag
		end
		local sec = handle.SectionName or "General"
		local lbl = handle.LabelName or (Config.Title or Config.Name or ctrlType or "Option")
		local baseKey = sec .. " / " .. lbl
		if ctrlType and ctrlType ~= "" then
			baseKey = baseKey .. " [" .. ctrlType .. "]"
		end
		local finalKey = baseKey
		local counter = 1
		while NeverLose.Flags[finalKey] do
			counter = counter + 1
			finalKey = baseKey .. " (" .. counter .. ")"
		end
		Config.Flag = finalKey
		return finalKey
	end
	...
	function handle:AddToggle(Config)
		...
		local flagKey = resolveFlag(Config, "Toggle");
		NeverLose.Flags[flagKey] = ToggleLib;
	...
	function handle:AddSlider(Config)
		...
		local flagKey = resolveFlag(Config, "Slider");
		NeverLose.Flags[flagKey] = SliderLib;
	...
	function handle:AddColorPicker(Config)
		...
		local flagKey = resolveFlag(Config, "ColorPicker");
		NeverLose.Flags[flagKey] = ColorPickerLib;
		ColorPickerLib.Flag = flagKey;
		ColorPickerLib.Type = "ColorPicker";
		ColorPickerLib.Default = Config.Default;

		function ColorPickerLib:Set(v)
			ColorPickerLib:SetValue(v)
		end;
	...
	function handle:AddKeybind(Config)
		...
		local flagKey = resolveFlag(Config, "Keybind");
		NeverLose.Flags[flagKey] = KeybindLib;
		KeybindLib.Flag = flagKey;
		KeybindLib.Type = "Keybind";
		KeybindLib.Default = Config.Default;

		function KeybindLib:Set(v)
			KeybindLib:SetValue(v)
		end;
	...
	function handle:AddTextInput(Config)
		...
		local flagKey = resolveFlag(Config, "TextInput");
		NeverLose.Flags[flagKey] = TextBoxLib;
		TextBoxLib.Flag = flagKey;
		TextBoxLib.Type = "TextInput";
		TextBoxLib.Default = Config.Default;
		TextBoxLib.Section = handle.SectionName;
		TextBoxLib.Title = handle.LabelName;

		function TextBoxLib:Set(v)
			TextBoxLib:SetValue(v)
		end;
	...
	function handle:AddDropdown(Config)
		...
		local flagKey = resolveFlag(Config, "Dropdown");
		NeverLose.Flags[flagKey] = DropdownLib;
		DropdownLib.Flag = flagKey;
		...
		function DropdownLib:Refresh(newValues)
			DropdownLib:SetValues(newValues)
		end;
...
function NeverLose:RegisiterItem(Frame , Signel)
	...
	function idx:AddToggle(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "Toggle"
		local handle = idx:AddLabel(title, false)
		if type(Config) == "table" and (Config.Description or Config.Content or Config.ToolTip) then
			pcall(function() handle:ToolTip(Config.Description or Config.Content or Config.ToolTip) end)
		end
		return handle:AddToggle(Config, ...)
	end

	function idx:AddSlider(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "Slider"
		local handle = idx:AddLabel(title, false)
		if type(Config) == "table" and (Config.Description or Config.Content or Config.ToolTip) then
			pcall(function() handle:ToolTip(Config.Description or Config.Content or Config.ToolTip) end)
		end
	...
	function idx:AddDropdown(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "Dropdown"
		local handle = idx:AddLabel(title, false)
		if type(Config) == "table" and (Config.Description or Config.Content or Config.ToolTip) then
			pcall(function() handle:ToolTip(Config.Description or Config.Content or Config.ToolTip) end)
		end
	...
	function idx:AddTextInput(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "Input"
		local handle = idx:AddLabel(title, false)
		if type(Config) == "table" and (Config.Description or Config.Content or Config.ToolTip) then
			pcall(function() handle:ToolTip(Config.Description or Config.Content or Config.ToolTip) end)
		end
	...
	function idx:AddColorPicker(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "ColorPicker"
		local handle = idx:AddLabel(title, false)
		return handle:AddColorPicker(Config, ...)
	end

	function idx:AddKeybind(Config, ...)
		local title = (type(Config) == "string" and Config) or (type(Config) == "table" and (Config.Title or Config.Name or Config[1])) or "Keybind"
		local handle = idx:AddLabel(title, false)
...
function NeverLose:CreateWindow(Config)
	...
	function Window:AddTab(Config)
		...
		function Tab:AddSection(Config)
			...
			SectionFrame.Size = UDim2.new(1, -5, 0, 20)
			...
			SectionLabel.Size = UDim2.new(1, -48, 0, 16)
			...
			ChevronIcon.Parent = SectionFrame
			ChevronIcon.AnchorPoint = Vector2.new(1, 0)
			ChevronIcon.BackgroundTransparency = 1.000
			ChevronIcon.Position = UDim2.new(1, -12, 0, 0)
			ChevronIcon.Size = UDim2.new(0, 16, 0, 16)
			ChevronIcon.ZIndex = 11
			ChevronIcon.FontFace = NeverLose.BuiltInBold
			ChevronIcon.Text = "chevron-small-down"
			ChevronIcon.TextColor3 = Color3.fromRGB(223, 223, 223)
			ChevronIcon.TextSize = 14.000
			ChevronIcon.TextTransparency = 0.500
			ChevronIcon.Active = false

			local HeaderButton = Instance.new("TextButton")
			HeaderButton.Name = "SectionHeaderButton"
			HeaderButton.Parent = SectionFrame
			HeaderButton.BackgroundTransparency = 1.000
			HeaderButton.Position = UDim2.new(0, 0, 0, 0)
			HeaderButton.Size = UDim2.new(1, 0, 0, 20)
			HeaderButton.Text = ""
			HeaderButton.AutoButtonColor = false
			HeaderButton.ZIndex = 12
			...
			local isCollapsed = Config.Collapsed == true or Config.defaultCollapsed == true

			local function updateSectionSize(animate)
				local tween = animate and VSlowTween or TweenInfo.new(0)
				if isCollapsed then
					SectionHandler.Visible = false
					NeverLose.PlayAnimate(ChevronIcon, tween, {
						Rotation = -90,
						TextColor3 = Color3.fromRGB(160, 160, 170)
					})
					NeverLose.PlayAnimate(SectionFrame, tween, {
						Size = UDim2.new(1, -5, 0, 20)
					})
				else
					SectionHandler.Visible = true
					NeverLose.PlayAnimate(ChevronIcon, tween, {
						Rotation = 0,
					...
					if UIListLayout.AbsoluteContentSize.Y <= 1 then
						...
						NeverLose.PlayAnimate(SectionFrame, tween, {
							Size = UDim2.new(1, -5, 0, UIListLayout.AbsoluteContentSize.Y + 23.5)
						})
					end
				end
			end
			...
			local function setCollapsed(val, anim)
				if isCollapsed == val then return end
				isCollapsed = val
				updateSectionSize(anim ~= false)
				if Config.OnCollapse then
					pcall(Config.OnCollapse, isCollapsed)
				end
			end

			NeverLose:AddSignal(HeaderButton.MouseEnter:Connect(function()
				NeverLose.PlayAnimate(SectionLabel, SlowyTween, {
					TextTransparency = 0.2
				})
				NeverLose.PlayAnimate(ChevronIcon, SlowyTween, {
					TextTransparency = 0.2,
					TextColor3 = NeverLose.AccentColor
				})
			end))

			NeverLose:AddSignal(HeaderButton.MouseLeave:Connect(function()
				NeverLose.PlayAnimate(SectionLabel, SlowyTween, {
					TextTransparency = 0.5
				})
				NeverLose.PlayAnimate(ChevronIcon, SlowyTween, {
					TextTransparency = 0.5,
					TextColor3 = isCollapsed and Color3.fromRGB(160, 160, 170) or Color3.fromRGB(223, 223, 223)
				})
			end))

			NeverLose:AddSignal(HeaderButton.MouseButton1Click:Connect(function()
				setCollapsed(not isCollapsed, true)
			end))
			...
			function Section:Collapse(anim)
				setCollapsed(true, anim ~= false)
			end
			function Section:Expand(anim)
				setCollapsed(false, anim ~= false)
			end
			function Section:Toggle(anim)
				setCollapsed(not isCollapsed, anim ~= false)
			end
			function Section:SetCollapsed(val, anim)
				setCollapsed(val, anim ~= false)
			end
		...
		function Tab:CollapseAll(anim)
			for _, s in ipairs(Tab.Sections or {}) do
				s:Collapse(anim)
			end
		end

		function Tab:ExpandAll(anim)
			for _, s in ipairs(Tab.Sections or {}) do
				s:Expand(anim)
			end
		end;
	...
	function Window:_InitConfig()
		...
		function ConfigLib:GetData(performance)
			...
			for flagKey, item in pairs(NeverLose.Flags) do
				if item and item.GetValue then
					local ok, val = pcall(function() return item:GetValue() end)
					if ok and val ~= nil then
						if typeof(val) == 'Color3' then
							valuesMap[flagKey] = "#" .. val:ToHex()
							table.insert(legacyList, { Idx = flagKey, Value = val:ToHex() })
						elseif typeof(val) == 'table' then
							local cloned = {}
							for k, v in pairs(val) do cloned[tostring(k)] = v end
							valuesMap[flagKey] = cloned
							table.insert(legacyList, { Idx = flagKey, Value = cloned })
						else
							valuesMap[flagKey] = val
							table.insert(legacyList, { Idx = flagKey, Value = val })
						end
					end
				end

				if performance and (cd % 35 == 1) then
					task.wait()
				end
				cd = cd + 1
			end
			...
			local payload = {
				...
				ActiveConfig = ConfigLib.SelectedConfig or "Default",
				SavedAt = (os.date and os.date("%Y-%m-%d %H:%M:%S")) or "Unknown",
				Values = valuesMap,
				List = legacyList
			}
		...
		function ConfigLib:LoadData(data)
			...
			local decoded = nil
			local jsonOk, res = pcall(function()
				return HttpService:JSONDecode(data)
			end)
			if jsonOk and type(res) == "table" then
				decoded = res
			else
				pcall(function()
					local raw = Encryption.reverse(NeverLose.Base64Decode(data))
			...
			if type(decoded.Values) == "table" then
				valuesToApply = decoded.Values
			elseif type(decoded.List) == "table" then
				for _, item in ipairs(decoded.List) do
					if type(item) == "table" and item.Idx then
						valuesToApply[item.Idx] = item.Value
					end
				end
			elseif #decoded > 0 then
				...
				valuesToApply = decoded
			end

			local appliedCount = 0
			for flagKey, savedVal in pairs(valuesToApply) do
				local control = NeverLose.Flags[flagKey]
				if control then
					task.spawn(function()
						pcall(function()
							if control.Type == "ColorPicker" and type(savedVal) == "string" then
								control:SetValue(Color3.fromHex(savedVal))
		...
		function Window:SaveConfig(name, isSilent)
			...
			if ok then
				...
				if not isSilent then
					Logging.new("floppy-disk", "Saved config '" .. name .. "'", 3)
				end
				return true
			end
			return false, err
		...
		function Window:LoadConfig(name, isSilent)
			...
			if not (isfile and isfile(path)) then
				if isfile and isfile(path .. ".json") then
					path = path .. ".json"
				else
					if not isSilent then
						Logging.new("triangle-exclamation", "Config '" .. name .. "' not found", 3)
					end
			...
			if ok and content and content ~= "" then
				local loadOk, count = ConfigLib:LoadData(content)
				if loadOk then
					ConfigLib.SelectedConfig = name
					ConfigName.Text = name
					UpdateSize()
					if not isSilent then
						Logging.new("folder", "Loaded config '" .. name .. "'", 3)
					end
					return true
				end
			end
		...
		function Window:DeleteConfig(name)
			...
			if isfile and isfile(path) then
				pcall(delfile, path)
				ConfigLib:RefreshConfig()
				Logging.new("trash-can", "Deleted config '" .. name .. "'", 3)
				return true
			end
			return false
		end;
		...
		function Window:GetConfigs()
			local list = {}
			local filesOk, files = pcall(listfiles, Window.ConfigFolder)
			if filesOk and type(files) == "table" then
		...
		function Window:GetActiveConfig()
			return ConfigLib.SelectedConfig or "Default"
		end;

		local autoSaveDebounce = false
		function Window:ScheduleAutoSave(delaySec)
			if not Window.AutoSaveEnabled then return end
			if autoSaveDebounce then return end
			autoSaveDebounce = true
			task.delay(delaySec or 1.5, function()
				autoSaveDebounce = false
				Window:SaveConfig(ConfigLib.SelectedConfig or "Default", true)
			end)
		end;

		function NeverLose:SaveConfig(name, isSilent)
			if NeverLose.ActiveWindow then
				return NeverLose.ActiveWindow:SaveConfig(name, isSilent)
			end
		end;

		function NeverLose:LoadConfig(name, isSilent)
			if NeverLose.ActiveWindow then
				return NeverLose.ActiveWindow:LoadConfig(name, isSilent)
			end
		end;

		function NeverLose:GetConfigs()
			if NeverLose.ActiveWindow then
				return NeverLose.ActiveWindow:GetConfigs()
			end
			return {"Default"}
		end;
		...
		function ConfigLib:RefreshConfig()
			...
			for i,ConfigNameStr in next , ConfigList do
				...
				local deleter,signal = NeverLose:CreateInput(DeleteConfig,function()
					Window:DeleteConfig(ConfigNameStr)
				end);

				local _,load_signal = NeverLose:CreateInput(LoadConfig,function()
					Window:LoadConfig(ConfigNameStr)
				end);
		...
		local hover_write = NeverLose:CreateInput(ConfigIcon,function()
			Window:SaveConfig(ConfigLib.SelectedConfig or "Default");
		end);
